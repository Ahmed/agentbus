...
def _write_json(path, record):
    """Atomically replace state without exposing held contents to other users."""
...
def _pending_path(client, confirmation_id):
    if not isinstance(confirmation_id, str) or not _CONFIRMATION_ID.fullmatch(confirmation_id):
        raise ValueError("invalid project confirmation id")
    return os.path.join(client.state, "pending.%s.json" % confirmation_id)
...
def _current_context(client, session, agent, generation=None):
    ...
    canonical = identity.canonical_session(client, session, generation)
    contexts = [_context(client, row["session"], agent, row["handle"], row.get("job"))
                for row in presence.routing_rows(client) if row["agent"] == agent]
    contexts = [context for context in contexts if context["session"] == canonical]
    if not contexts:
        return None
...
def _canonical_context(client, context):
    ...
    if generation is not None or not re.fullmatch(r"(claude|codex|gemini|node)[0-9]+", session):
        result["session"] = identity.canonical_session(client, session, generation)
...
def _pair(contexts):
    ordered = sorted(contexts, key=lambda item: (item["session"], item["agent"]))
...
def _relationship(client, first, second, confirmation_id=None, revoke=False):
    ...
    with open(path + ".lock", "a") as guard:
        ...
        for previous_key, previous in list(links.items()):
            if not isinstance(previous, dict) or len(previous.get("contexts", [])) != 2:
                continue
            canonical = [_canonical_context(client, item) for item in previous["contexts"]]
            canonical_key, canonical = _pair(canonical)
            if canonical_key == previous_key and canonical == previous["contexts"]:
                continue
        ...
        if confirmation_id is not None:
            links[key] = {"contexts": contexts, "confirmation_id": confirmation_id,
...
def invalidate_relationships(client):
    ...
    with open(path + ".lock", "a") as guard:
        fcntl.flock(guard, fcntl.LOCK_EX)
        links = _load_json(path) or {}
        stale = [key for key, entry in links.items()
                 if isinstance(entry, dict)
                 and any(_canonical_context(client, context)["session"] == session
...
def _task_status(client, payload, status, confirmation_id=None):
    """Advance delegation state only when its held contents become available."""
    ...
    if payload["kind"] == "result":
        ...
        if task_state == "sent" and current.get("status") in ("delivered", "done"):
            task_state = current["status"]
        fields = {"status": task_state, "confirmation_status": status,
                  "confirmation_id": confirmation_id,
                  "sender": payload["from"], "sender_handle": payload.get("from_handle"),
                  "sender_session": payload.get("from_session"),
                  "sender_session_start": payload.get("from_session_start"),
                  "assignee": payload["to"], "assignee_handle": payload["to"],
                  ...
                  "requested_assignee": payload.get("requested_to", payload["to"]),
...
def _project(client, record):
    ...
    return client.job if not project or project == constants.BROADCAST_JOB else project


def _probe(client, sender, record, confirmation_id, project):
    task = routing.task_stem(record.get("from_handle"), sender, record["from_session"])
    description = " Sender task: %r." % task if task else ""
    receiver = record.get("to_agent") or record["to"]
    command = "python3 %s confirm %s %s yes" % (
        shlex.quote(os.path.join(os.path.dirname(__file__), "bus.py")), shlex.quote(receiver),
        shlex.quote(confirmation_id))
    question = (
        "Are you working on project %r?%s %s wants to share a message; "
        "its contents are withheld until you explicitly confirm. "
        "Reply yes only if this window is working on that project, otherwise no. "
        "Confirm with: %s. To decline, replace yes with no. "
        "Alternatively use the MCP confirm_project tool "
        "with confirmation_id=%r and accept=true or false."
        % (project, description, record.get("from_handle", sender), command,
           confirmation_id)
    )
    check = {
        "id": confirmation_id, "ts": time.time(), "from": sender,
        "from_handle": record.get("from_handle", state.current_handle(client, sender)),
        "from_session": record["from_session"], "to": record["to"],
        "kind": "project_check", "text": question, "job": project,
        "project": project, "confirmation_id": confirmation_id,
        "requested_to": record.get("requested_to", record["to"]),
        "routing": "project_confirmation",
    }
    for key in ("to_session", "to_session_start", "to_agent", "from_session_start"):
        if record.get(key):
...
def _request_one(client, sender, record):
    ...
    if payload.get("to_session") and payload.get("to_agent"):
        target_context = _current_context(client, payload["to_session"], payload["to_agent"],
    ...
    with open(path + ".lock", "a") as guard:
        ...
        storage.compact(client)
    result = _metadata(payload)
    result.update(status="awaiting_confirmation", confirmation_id=confirmation_id)
...
def request_confirmation(client, sender, record):
    ...
    if record.get("kind") not in ("message", "task", "result"):
        raise ValueError("project confirmation applies to messages, tasks and results")
    if record.get("from") != sender or not record.get("from_session"):
        raise ValueError("project confirmation requires the sending window identity")
    ...
    for (session, agent), row in sorted(windows.items()):
        payload = dict(record)
        payload.update(id=_new_id(), to=row["handle"], to_session=session, to_agent=agent)
        generation = identity.session_generation(session)
    ...
    result["status"] = ("queued" if all(item["status"] == "queued" for item in confirmations)
...
def _status_notice(client, agent, pending):
    ...
    return {
        "id": _new_id(), "ts": time.time(), "from": agent,
        "from_handle": state.current_handle(client, agent), "from_session": client.session,
        "to": payload.get("from_handle", payload["from"]),
        "to_session": identity.canonical_session(client, payload["from_session"],
                                               payload.get("from_session_start")),
        "to_session_start": payload.get("from_session_start"),
        "to_agent": payload["from"], "kind": "project_status",
        "text": "Project confirmation %s %s." % (pending["confirmation_id"], outcome),
        "job": pending["project"], "project": pending["project"],
        "confirmation_id": pending["confirmation_id"],
        "message_id": payload["id"], "status": status,
        "reply_to": pending["confirmation_id"], "routing": "project_confirmation",
    }
...
def _confirmation_result(pending):
    ...
    result = {"status": pending["status"], "id": payload["id"],
              "to": payload["to"], "confirmation_id": pending["confirmation_id"]}
...
def confirm_project(client, agent, confirmation_id, accept):
    ...
    with open(path + ".lock", "a") as guard:
        ...
        if pending is None:
            raise ValueError("unknown project confirmation %r" % confirmation_id)
        if not routing.addressed_to(client, agent, pending["probe"]):
            raise ValueError("project confirmation is addressed to another window")
        ...
        already_released = released is not None and _already_appended(client, released["id"])
        source = _canonical_context(client, pending["source_context"])
        current_source = _current_context(client, source["session"], source["agent"],
        ...
        if not already_released and time.time() >= pending["expires_at"]:
        ...
        elif not already_released and pending.get("decision") is None and not accept:
            ...
            if released is None:
                ...
                released.update(ts=time.time(), to=state.current_handle(client, agent),
                                to_session=client.session, to_agent=agent,
                                from_session=source["session"],
                                confirmation_id=confirmation_id)
                pending.update(status="releasing", decision=True, release=released,
            ...
            if pending["project"] == source["job"]:
                _relationship(client, source, pending.get("recipient_context", recipient),
                              confirmation_id=confirmation_id)
            pending["status"] = "confirmed"
        if pending["status"] in ("rejected", "expired"):
            _task_status(client, pending["payload"], pending["status"], confirmation_id)
        _write_json(path, pending)
        _notify(client, agent, path, pending)
        return _confirmation_result(pending)
